...
Ilich, Alexander. 2020. "GLCMTextures.", doi:http://doi.org/10.5281/zenodo.4310187. https://github.com/ailich/GLCMTextures. 
...
Then to install this package use the code `devtools::install_github("ailich/GLCMTextures")`
...
There are two methods of quantization available in the quantize_raster function. `method = "equal range"` will create bins that cover a range of equal size (e.g. if the original data ranged from 0-20 and was quantized to 4 levels, [0-5) would be reassigned to 0, [5-10) would be reassigned to 1, [10-15) would be reassigned to 2, and [15-20] would be reassigned to 4). This is the simplest and most common quantization method. `method = "equal prob"` performs equal probability quantization and will use quantiles (Hyndman and Fan 1996) to create bins that contain an approximately equal number of samples, which is the quantization method suggested in the original paper (Haralick and Shanmugam 1973).
