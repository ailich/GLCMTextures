...
glcm_textures<- function(r, w, n_levels, shift=list(c(1,0), c(1,1), c(0,1), c(-1,1)), metrics= c("glcm_contrast", "glcm_dissimilarity", "glcm_homogeneity", "glcm_ASM", "glcm_entropy", "glcm_mean", "glcm_variance", "glcm_correlation"), quantization, min_val=NULL, max_val=NULL, na_opt= "any", pad=FALSE){
  ...
  #BLOCKING METHOD USING GET/WRITEVALUES RATHER THAN CROP AND MERGE
  #GETTING SLIGHTLY DIFFERENT VALUES USING GET/WRITEVALUES
  #STILL HAVE OVERHEAD RELATED TO HELPER FUNCTION USING RASTER RATHER THAN MATRIX
  # else{
  ...
  #   max_row<- min(c(block_idx$row[[i]] + block_idx$nrows[[i]] - 1 + block_overlap, nrow(r)))
  #   curr_block <- raster::raster(raster::getValues(r, row = min_row, nrows = max_row-min_row+1, format="matrix"))
  #
  #   if(i==1){
  #     min_row_chunk<- 1} else{
  #       min_row_chunk<- block_overlap+1}
  #
  #   out_block<- raster::getValues(glcm_textures_helper(rq=curr_block, w=w, n_levels=n_levels, shift=shift, metrics=metrics, na_opt=na_opt),row = min_row_chunk, nrows = block_idx$nrows[i]) #Maybe edit glcm_textures_helper to take and return a matrix
  #
  #   output<- raster::writeValues(output, v= out_block, start= block_idx$row[i])
  # }
  # raster::writeStop(output)
  # names(output)<- metrics}
