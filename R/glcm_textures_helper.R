...
glcm_textures_helper<- function(rq, w, n_levels, shift=list(c(1,0), c(1,1), c(0,1), c(-1,1)), metrics= c("glcm_contrast", "glcm_dissimilarity", "glcm_homogeneity", "glcm_ASM", "glcm_entropy", "glcm_mean", "glcm_variance", "glcm_correlation")){
  ...
  for (j in 1:n_layers) {
    ...
    if(j==1){
      out_rasts<- out_layer} else{
        out_rasts<- raster::stack(out_rasts, out_layer)} #Create new stack of averaged values
  }
  names(out_rasts)<- metrics
  if(n_layers>1){
    out_rasts<- raster::brick(out_rasts)}
  return(out_rasts)
}